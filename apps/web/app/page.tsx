...
import { MapPinIcon, UsersIcon, MapIcon, BotIcon, PackageIcon, ZapIcon, ShieldAlertIcon, HeartHandshakeIcon, LockIcon } from "lucide-react";
    ...
    <div className="min-h-screen">
      ...
      <section className="flex flex-col items-center justify-center gap-6 px-4 pt-24 pb-20 text-center">
        ...
        <div className="flex gap-3 mt-2">
          ...
          <Link href="/dashboard" className={cn(buttonVariants({ variant: user ? "default" : "outline", size: "lg" }), "text-base px-8")}>
            View Dashboard
          </Link>
        </div>
      </section>
      ...
      <section className="container mx-auto max-w-5xl px-4 py-16">
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground max-w-lg mx-auto">Everything you need for effective disaster response coordination.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {features.map((f) => (
            <Card key={f.title} className="transition-transform hover:-translate-y-1">
              <CardContent className="pt-6 space-y-3">
                <div className={`inline-flex items-center justify-center size-12 ${f.color}`}>
      ...
      <section className="container mx-auto max-w-4xl px-4 py-16">
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground">Whether you need help or want to provide it.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {roles.map((r) => (
            <Card key={r.title} className="text-center transition-transform hover:-translate-y-1">
              <CardContent className="pt-6 space-y-4">
                <div className={`inline-flex items-center justify-center size-14 mx-auto ${r.badgeColor}`}>
                  <r.icon className="size-7" />
                </div>
                <div>
                  <h3 className="font-bold text-lg">{r.title}</h3>
                  <Badge variant="outline" className="mt-1">{r.badge}</Badge>
                </div>
      ...
      <section className="container mx-auto max-w-2xl px-4 py-16">
        <Card className="bg-primary text-primary-foreground">
          <CardContent className="py-10 text-center space-y-4">
            ...
            <p className="opacity-90">Join ReachOut and help coordinate disaster relief in your area.</p>
            <div className="flex justify-center gap-3 pt-2">
              <Link href={user ? "/dashboard" : "/register"} className={cn(buttonVariants({ variant: "secondary", size: "lg" }), "text-base font-bold px-8")}>
