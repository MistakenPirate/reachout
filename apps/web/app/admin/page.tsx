...
function OverviewCards() {
	...
	const cards = [
		...
		},
		{
			...
			value: s.availableVolunteers,
			color: "text-green-600",
		},
		{
			label: "Deployed",
			value: s.onMissionVolunteers,
				...
				<div className="space-y-3 max-h-[600px] overflow-y-auto">
					...
					{filtered.map((req) => (
						<Card
							...
							onClick={() =>
								req.status === "pending" && setSelectedRequestId(req.id)
							}
						>
							...
							<Card key={vol.id}>
								<CardContent className="p-4 flex items-center justify-between">
									<div>
										...
										<p className="text-sm text-muted-foreground">
											{vol.distance.toFixed(1)} km &middot;{" "}
											{vol.skills.join(", ") || "No skills"}
										</p>
									</div>
									<Button
										size="sm"
				...
				{filtered.map((vol) => (
					...
					>
						<div>
							<p className="font-medium">{vol.userName || "Unknown"}</p>
							<p className="text-sm text-muted-foreground">
								Skills: {vol.skills.length > 0 ? vol.skills.join(", ") : "None"}{" "}
								&middot;
								{vol.latitude != null
									? ` ${vol.latitude.toFixed(2)}, ${vol.longitude?.toFixed(2)}`
									: " No location"}
							</p>
						</div>
						<Badge
				...
				<CardContent>
					<form onSubmit={handleSubmit} className="space-y-4">
						<div className="grid grid-cols-2 gap-3">
							<div className="space-y-2">
								...
								<Input
									...
									required
									placeholder="e.g. Chennai Flood Zone"
								/>
							</div>
							<div className="space-y-2">
						...
						<div className="grid grid-cols-2 gap-3">
							...
							<div className="space-y-2">
								<Label htmlFor="zrad">Radius (km)</Label>
								<Input
									id="zrad"
									type="number"
									step="any"
						...
						<div className="grid grid-cols-2 gap-3">
							...
							<div className="space-y-2">
								...
								<Select
									value={type}
									onValueChange={(v) => v && setType(v as EmergencyType)}
								>
									<SelectTrigger>
										<SelectValue />
									</SelectTrigger>
			...
			{zones.length > 0 && (
				<Card>
					...
					<CardContent>
							...
							{zones.map((z) => (
								...
								>
									...
									<div className="flex items-center gap-2">
										...
										<Badge
											variant={
												z.severity === "critical" ? "destructive" : "secondary"
											}
										>
											{z.severity}
										</Badge>
						...
						<PaginationControls
							page={zonePage}
							totalPages={zonesData?.totalPages ?? 1}
							total={zonesData?.total ?? 0}
							setPage={setZonePage}
						/>
					</CardContent>
				</Card>
...
function ResourcesPanel() {
	...
	const resources = (resData?.data ?? []) as {
		id: string;
		name: string;
		type: string;
		quantity: number;
					...
					<form onSubmit={handleCreate} className="space-y-4">
						<div className="grid grid-cols-2 gap-3">
							...
							<div className="space-y-2">
								...
								<Select
									value={type}
									onValueChange={(v) => v && setType(v as ResourceType)}
								>
									<SelectTrigger>
										<SelectValue />
									</SelectTrigger>
									<SelectContent>
										...
										<SelectItem value="shelter_kit">Shelter Kit</SelectItem>
										<SelectItem value="other">Other</SelectItem>
									</SelectContent>
								</Select>
							</div>
								...
								{resources.map((res) => (
									...
									>
										<div className="flex items-center gap-2">
											...
											<Badge
												variant={res.quantity > 0 ? "outline" : "destructive"}
											>
												Qty: {res.quantity}
											</Badge>
										</div>
										{allocateId === res.id ? (
											<div className="flex items-center gap-2">
												...
												<Button
													...
													disabled={
														allocateMutation.isPending || !allocateAmount
													}
												>
													OK
												</Button>
		...
		<div className="space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Run AI to score and rank all pending help requests by urgency.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-3">
					...
					{prioritize.data && prioritize.data.length === 0 && (
						<p className="text-sm text-muted-foreground">
							No pending requests to prioritize.
						</p>
					)}
				</CardContent>
			</Card>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Search the web for real disaster reports and analyze damage.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-3">
					<div className="flex gap-2">
						...
						<Button
							onClick={() => {
								const timeCode: Record<string, string> = {
									"Past 24h": "d",
									"Past Week": "w",
									"Past Month": "m",
								};
								summarize.mutate(
									{ keyword, timeRange: timeCode[timeRange] ?? "w" },
							...
							{summary.sources && summary.sources.length > 0 && (
								<div>
									<p className="text-sm font-medium text-muted-foreground mb-1">
										Web Sources ({summary.sources.length})
									</p>
