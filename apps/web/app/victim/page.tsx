...
function VictimPageContent() {
	...
	const { data: rescueStatuses = [], isLoading: rescueLoading } =
		useRescueStatus();
						...
						<div className="space-y-6 pt-4">
							<Card>
								<CardHeader>
									...
									<CardDescription>
										Submit your location and emergency details so rescuers can
										find you.
									</CardDescription>
								</CardHeader>
								<CardContent>
									<form onSubmit={handleSubmit} className="space-y-4">
										...
										<div className="space-y-2">
											...
											<Select
												value={emergencyType}
												onValueChange={(v) => v && setEmergencyType(v)}
											>
												<SelectTrigger>
													<SelectValue />
												</SelectTrigger>
											...
											{(reqData?.totalPages ?? 1) > 1 && (
												<div className="flex items-center justify-between pt-2">
													...
													<div className="flex items-center gap-2">
														...
														<span className="text-sm text-muted-foreground">
															{reqPage + 1} / {reqData?.totalPages ?? 1}
														</span>
														<Button
															size="sm"
															variant="outline"
															onClick={() => setReqPage((p) => p + 1)}
															disabled={
																reqPage >= (reqData?.totalPages ?? 1) - 1
															}
														>
															Next
														</Button>
						...
						<div className="space-y-6 pt-4">
							<Card>
								<CardHeader>
									...
									<CardDescription>
										Track the status of your active rescue requests in
										real-time.
									</CardDescription>
								</CardHeader>
								<CardContent>
												...
												>
													<div className="flex items-center justify-between">
														<div className="flex items-center gap-2">
															...
															<Badge
																variant={
																	statusVariant[rescue.status] || "outline"
																}
															>
															...
															<div className="space-y-1">
																...
																<div className="grid grid-cols-2 gap-2 text-sm text-muted-foreground">
																	...
																	{rescue.volunteer.distance !== null && (
																		<span>
																			Distance:{" "}
																			{rescue.volunteer.distance.toFixed(1)} km
																		</span>
																	)}
																</div>
															</div>
