...
async function fetchMyRequestsPaginated(page: number, limit: number): Promise<PaginatedResponse<HelpRequest>> {
  const res = await fetch(`${API_URL}/api/help-requests/mine?page=${page}&limit=${limit}`, { headers: authHeaders() });
