...
export async function createHelpRequest(data: {
	...
	if (!res.ok)
		throw new Error(
			typeof json.error === "string" ? json.error : "Failed to create request",
		);
	return json;
}
...
export async function resolveHelpRequest(id: string) {
	...
	if (!res.ok)
		throw new Error(
			typeof json.error === "string" ? json.error : "Failed to resolve request",
		);
	return json;
}
...
export async function fetchVolunteerMissions() {
	const res = await fetch(`${API_URL}/api/volunteers/missions`, {
		headers: authHeaders(),
	});
...
export async function updateMissionStatus(id: string, status: string) {
	...
	if (!res.ok)
		throw new Error(
			typeof json.error === "string" ? json.error : "Failed to update mission",
		);
	return json;
}
