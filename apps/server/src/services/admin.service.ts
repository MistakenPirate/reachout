...
export async function getSuggestedVolunteers(requestId: string) {
	...
	const volunteers =
		await disasterRepo.findAllAvailableVolunteersWithLocation();
...
export async function assignVolunteer(
	...
	if (request.status !== "pending")
		throw new ServiceError(400, "Request is not pending");

	const [updated] = await disasterRepo.assignVolunteerToRequest(
		requestId,
		volunteerUserId,
	);
...
export async function allocateResource(
	...
	if (resource.quantity < amount)
		throw new ServiceError(400, "Insufficient quantity");

	const [updated] = await disasterRepo.decrementResourceQuantity(
		resourceId,
		amount,
	);
