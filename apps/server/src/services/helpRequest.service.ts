...
export async function getUserRequestsPaginated(userId: string, page: number, limit: number) {
...
export async function resolveRequest(requestId: string, userId: string) {
  ...
  if (request.userId !== userId) throw new ServiceError(403, "Not your request");

  const [updated] = await disasterRepo.updateHelpRequestStatus(requestId, "resolved");
