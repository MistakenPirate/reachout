...
export function findAllActiveZones() {
	return db
		.select()
		.from(disasterZones)
		.where(eq(disasterZones.status, "active"));
}
...
export function updateVolunteer(
	...
	data: Partial<
		Pick<
			typeof volunteers.$inferInsert,
			"skills" | "latitude" | "longitude" | "isAvailable" | "status"
		>
	>,
) {
	return db
...
export function assignVolunteerToRequest(
	requestId: string,
	volunteerId: string,
) {
	return db
...
export function findAllAvailableVolunteersWithLocation() {
	return db
		...
		.leftJoin(users, eq(volunteers.userId, users.id))
		.where(
			and(eq(volunteers.isAvailable, true), eq(volunteers.status, "available")),
		);
}
...
export function findHelpRequestsByUserPaginated(
	userId: string,
	limit: number,
	offset: number,
) {
