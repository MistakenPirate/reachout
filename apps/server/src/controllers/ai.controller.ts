...
export const prioritizeRequests: RequestHandler = async (_req, res) => {
	try {
		...
		await Promise.all(
			results.map((r) =>
				disasterRepo.updateHelpRequestPriority(r.requestId, r.score),
			),
		);
...
export const summarizeSocialMedia: RequestHandler = async (req, res) => {
	...
	try {
		const summary = await aiService.summarizeSocialMedia(
			keyword,
			timeRange || "w",
		);
		res.json(summary);
...
async function buildVictimContext(userId: string): Promise<string> {
	...
	for (const req of requests) {
		lines.push(
			`- Request: ${req.emergencyType}, ${req.peopleCount} people, status: ${req.status}, location: (${req.latitude}, ${req.longitude})`,
		);
		...
		if (req.assignedVolunteerId) {
			...
			const [volUser] = await disasterRepo.findUserById(
				req.assignedVolunteerId,
			);
			if (vol && volUser) {
				const dist =
					vol.latitude != null && vol.longitude != null
						...
						: "unknown";
				lines.push(
					`  Assigned volunteer: ${volUser.name}, phone: ${volUser.phone}, status: ${vol.status}, distance: ${dist} km, available: ${vol.isAvailable}`,
				);
			}
		} else {
...
async function buildVolunteerContext(userId: string): Promise<string> {
	const missions =
		await disasterRepo.findHelpRequestsAssignedToVolunteer(userId);
	const [profile] = await disasterRepo.findVolunteerByUserId(userId);

	const lines: string[] = [];
	if (profile) {
		lines.push(
			`YOUR PROFILE: skills: ${profile.skills.join(", ") || "none"}, status: ${profile.status}, available: ${profile.isAvailable}, location: ${profile.latitude != null ? `(${profile.latitude}, ${profile.longitude})` : "not set"}`,
		);
	}
		...
		for (const m of missions) {
			const dist =
				profile?.latitude != null && profile?.longitude != null
					...
					: "unknown";
			lines.push(
				`- ${m.emergencyType} emergency, ${m.peopleCount} people, status: ${m.status}, victim location: (${m.latitude}, ${m.longitude}), distance: ${dist} km`,
			);
			if (m.userName)
				lines.push(
					`  Victim: ${m.userName}${m.userPhone ? `, phone: ${m.userPhone}` : ""}`,
				);
