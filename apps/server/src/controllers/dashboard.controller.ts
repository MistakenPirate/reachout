  ...
  const result = await Promise.all(
    active.map(async (request) => {
      ...
      if (request.assignedVolunteerId) {
        const [user] = await disasterRepo.findUserById(request.assignedVolunteerId);
        const [vol] = await disasterRepo.findVolunteerByUserId(request.assignedVolunteerId);
        ...
        if (user && vol && vol.latitude != null && vol.longitude != null) {
          ...
          const a =
            ...
            Math.cos((request.latitude * Math.PI) / 180) *
              ...
              Math.sin(dLon / 2) ** 2;
          volunteerDistance = R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));

          volunteer = { name: user.name, phone: user.phone, distance: Math.round(volunteerDistance * 10) / 10 };
        } else if (user) {
